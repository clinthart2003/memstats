...
for host in CONFIG.graphhost:
    ...
    if connection is True:
        ...
        try:
            cmd = r'cat %s/threatq-memory-%s.log | grep "DATE:"' % (remotepath, host)
            tvalue = grep_time(cmd)
            cmd = r'cat %s/threatq-memory-%s.log | grep "tq-supervisord" | grep threatq' % (remotepath, host)
            tqcontroller = grep_memory(cmd, "NONE")
            header = ["time",  "PID", "USER", "PR", "NI", "VIRT", "RES", "SHR", "S", "%CPU", "%MEM", "TIME+", "COMMAND"]
        ...
        try:
            cmd = r'cat %s/threatq-memory-%s.log | grep "DATE:"' % (remotepath, host)
            tvalue = grep_time(cmd)
            cmd = r'cat %s/threatq-memory-%s.log | grep "/python/bin/dynamo"' % (remotepath, host)
        ...
        try:
            cmd = r'cat %s/threatq-memory-%s.log | grep "DATE:"' % (remotepath, host)
            tvalue = grep_time(cmd)
            cmd = r'cat %s/threatq-memory-%s.log | grep "systemd-journald"' % (remotepath, host)
        ...
        try:
            cmd = r'cat %s/threatq-memory-%s.log | grep "DATE:"' % (remotepath, host)
            tvalue = grep_time(cmd)
            cmd = r'cat %s/threatq-memory-%s.log | grep "java -server"' % (remotepath, host)
        ...
        try:
            cmd = r'cat %s/threatq-memory-%s.log | grep "DATE:"' % (remotepath, host)
            tvalue = grep_time(cmd)
            cmd = r'cat %s/threatq-memory-%s.log | grep "worker"' % (remotepath, host)
