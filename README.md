...
There are three versions of this graphing framework.   A matplotlib, mpld3, and plotly.   There is a "runit.bat" file that shows the execution of the different scripts in order.   All log files are accessed via SSH, parsed and then normalized CSV files are created.  From there the graphig scripts can read each CSV file and generate the graphs.  There are example generated files in the attached directory.  
